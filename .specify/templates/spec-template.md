...
- **FR-004**: Implementation MUST provide real-time updates via Socket.IO if user-facing state changes.
- **FR-005**: All new API endpoints MUST be documented in `docs/API.md`.
- **FR-006**: If the feature exposes a public/embedded surface, it MUST authenticate via API key (or equivalent) and MUST include an abuse-control plan (domain allow-listing and/or rate limiting).
